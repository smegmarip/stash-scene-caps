...
(function () {
  ...
  async function getTimeOffset(spriteUrl, spritePos, mouseCoords) {
    // get vtt blob
    ...
    waitForElm(wrapper).then(async ($el) => {
      ...
      if (spriteUrl) {
        if (!document.querySelector("#screencaps")) {
          ...
          link.setAttribute(
            "style",
            "display: block; background:transparent; position: absolute; top: 10px; left: 10px; bottom: 10px; right: 10px; cursor: pointer;"
          );
          link.addEventListener("click", function (e) {
            const mouseCoords = { x: e.offsetX, y: e.offsetY };
            bgImageSize(screenCaps, spriteUrl, (spritePos) => {
              getTimeOffset(spriteUrl, spritePos, mouseCoords);
            });
          });
